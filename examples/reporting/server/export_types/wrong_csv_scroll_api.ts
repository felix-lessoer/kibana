...
export const CSV_REPORT_TYPE_SCROLL = 'csv_scroll';

type CsvScrollApiExportTypeSetupDeps = BaseExportTypeSetupDeps;
export interface CsvScrollApiExportTypeStartDeps extends BaseExportTypeStartDeps {
  discover: DiscoverServerPluginStart;
  data: DataPluginStart;
}

export class CsvScrollApiExportType extends ExportType<
  JobParamsCsvFromSavedObject,
  TaskPayloadCsvFromSavedObject,
  CsvScrollApiExportTypeSetupDeps,
  CsvScrollApiExportTypeStartDeps
> {
  id = CSV_REPORT_TYPE_SCROLL;
  name = CSV_REPORT_TYPE_SCROLL;
  jobType = CSV_REPORT_TYPE_SCROLL;
  ...
  constructor(...args: ConstructorParameters<typeof ExportType>) {
    super(...args);
    const logger = args[2];
    this.logger = logger.get('csv-export-scroll');
  }
  ...
  public createJob = async (
    ...
    console.log("Create job from the scroll export type");
    if (
      !locatorParams ||
